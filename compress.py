...
def to_apng(input_file, output_file):
    os.system("mkdir -p %s" % tmp_dir)

    cmd = "convert %s -coalesce %s/xx_%%05d.png" % (input_file, tmp_dir)
    os.system(cmd)

    cmd = "./pngquant -f --ext .png --quality=65-80 %s/*.png" % tmp_dir
    os.system(cmd)

    cmd = "./apngasm %s %s/xx_*.png" %(output_file, tmp_dir)
    os.system(cmd)
...
for f in os.listdir(input_dir):
    ...
    output_f = output_f.replace(".gif", ".png")

    to_apng(input_f, output_f)
